...
def test_main(
    ...
    for token_idx in range(num_tokens):
        for k in range(num_topk):
            # expert_idx = (((token_idx % 96) * num_topk) + k)

            expert_idx = (curr_rank * num_experts_per_rank) + offset
            curr_rank += 1
            if curr_rank == num_ranks:
                curr_rank = 0
                offset = (offset + 1) % num_experts_per_rank
            scores[token_idx, expert_idx] = num_topk - k
    # print(scores)
    ...
    for r in range(num_ranks):
        ...
        #     + 1
        # )
        r_scores = torch.zeros((num_tokens, num_experts), dtype=torch.float32, device="cuda")
        curr_rank = 0
        offset = 0
        for token_idx in range(num_tokens):
            for k in range(num_topk):
                ...
                if curr_rank == num_ranks:
                    curr_rank = 0
                    offset = (offset + 1) % num_experts_per_rank
                r_scores[token_idx, expert_idx] = num_topk - k
                #print(r_scores)
